			...
			for (int i = 0; i < maxLog; i++) {
				if (MasterServer.nameList[i] != null) {
					if (MasterServer.nameList[i].equalsIgnoreCase(name)) {
						MasterServer.serverList[i].sendMessage("(Private)[" + currentUser + "]: " + input);
		...
		default: {
			...
			for(int i = 0; i < maxLog; i++){
				if(MasterServer.serverList[i] != null){
					if(checkWords == 0){
					MasterServer.serverList[i].sendMessage("[" + currentUser + "]: " + firstWord + " " + input);
					}
					else{
						MasterServer.serverList[i].sendMessage("[" + currentUser + "]: " + firstWord);
					}
				}
			}
			break;
